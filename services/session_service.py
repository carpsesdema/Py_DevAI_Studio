...
class SessionService:
                            ...
                            for item_dict in history_list_raw:
                                try:
                                    ...
                                    if isinstance(parts_raw, str):
                                        ...
                                        parts_list = [p for p in parts_raw if isinstance(p, (str, dict))]
                                    else:
                                        parts_list = []
                                    deserialized_history.append(
                                        ChatMessage(role=str(role), parts=parts_list, timestamp=timestamp,
                                                    metadata=metadata,
                                                    id=msg_id))  # , loading_state=loading_state_enum))
                                except Exception as e_msg:
                                    logger.warning(
                                        f"Error deserializing ChatMessage item in project '{pid}' from {filepath}: {e_msg}. Skipping item.")
                    ...
                    for item_dict in raw_history:
                        try:
                            ...
                            if isinstance(parts_raw, str):
                                ...
                                parts_list = []
                            deserialized_history.append(
                                ChatMessage(role=str(role), parts=parts_list, timestamp=timestamp, metadata=metadata,
                                            id=msg_id))
                        except Exception:
                            pass
    ...
    def _chatmessage_to_dict(self, msg: ChatMessage) -> Dict[str, Any]:
        ...
        return {"role": msg.role, "parts": msg.parts, "timestamp": msg.timestamp, "metadata": serializable_metadata,
                "id": msg.id}  # , "loading_state": loading_state_str}
    ...
    def save_last_session(self,
        ...
        if session_extra_data and isinstance(session_extra_data, dict):
            ...
            logger.debug(
                f"Merged session_extra_data into save data for last session: {list(session_extra_data.keys())}")
    ...
    def clear_last_session_file(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Error deleting last session file {constants.LAST_SESSION_FILEPATH}: {e}");
            return False
        except Exception as e:
            logger.exception(f"Unexpected error clearing last session file: {e}");
            return False
    ...
    def list_sessions(self) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not sort by mtime: {sort_e}");
                full_paths.sort(key=os.path.basename,
                                reverse=True)
