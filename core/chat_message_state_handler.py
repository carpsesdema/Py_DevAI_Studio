...
class ChatMessageStateHandler(QObject):
    ...
    def _handle_response_error(self, request_id: str, error_message: str):
        ...
        """
        logger.info(
            f"StateHandler: Response error for request_id '{request_id}'. Error: '{error_message}'. Setting state to COMPLETED (or ERROR).")
        ...
        success = self._model.update_message_loading_state_by_id(request_id,
                                                                 MessageLoadingState.COMPLETED)  # Or MessageLoadingState.ERROR
        if not success:
